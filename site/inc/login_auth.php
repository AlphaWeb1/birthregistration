...
if (!empty($posts) and !empty($posts->sitePage)) {
	...
	if (empty($userDetail)) {
		$pword=getToken(10);
		$ezDb->query("INSERT INTO `userprofile` (`firstname`, `lastname`, `email`, `password`, `username`, `terms`, `active`, `dateadded`, `usertype`, `usercat`, `userrole`, `verified`, `instructor`) VALUES ('$posts->firstname', '$posts->lastname', '".strtolower($posts->email)."', '".base64_encode($pword)."', '".strtolower($posts->email)."', '1', '1', '$dateNow', 'client', 'individual', 'level1', '1', '0');");
